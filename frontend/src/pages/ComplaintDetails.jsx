...
export default function ComplaintDetails() {
  ...
  if (!complaint) return <div className="glass rounded-2xl p-6">Complaint not found.</div>;

  return (
    <div className="glass rounded-2xl p-6 shadow-card space-y-4">
      <h2 className="text-2xl font-heading font-semibold">Complaint #{complaint.id}</h2>
      <span className={`px-3 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[complaint.status]}`}>{complaint.status.replace('_', ' ')}</span>
      <div>
      ...
      {user?.role === 'CITIZEN' && complaint.status === 'RESOLVED' && (
        <div className="flex gap-3">
          <Link to={`/feedback/${complaint.id}`} className="px-4 py-2 rounded-lg bg-primary text-white">Rate Service</Link>
          <button onClick={reopenComplaint} className="px-4 py-2 rounded-lg bg-rose-100 text-rose-700">Reopen Complaint</button>
        </div>
      )}
    </div>
  );
